...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add "Send for Approval" buttons to list view
    function addListApprovalButtons() {
        const actionButtons = document.querySelectorAll('.field-send_approval');
        actionButtons.forEach(function(button) {
            if (!button.querySelector('.send-approval-btn')) {
                const sendBtn = document.createElement('button');
                sendBtn.type = 'button';
                sendBtn.className = 'btn btn-sm btn-warning send-approval-btn';
                sendBtn.textContent = 'Send for Approval';
                sendBtn.style.marginRight = '5px';
                
                sendBtn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const heroId = this.closest('tr').querySelector('input[name="_selected_action"]').value;
                    sendForApproval(heroId);
                });
                
                button.appendChild(sendBtn);
            }
        });
    }
    ...
    if (window.location.pathname.includes('/add/')) {
        ...
        // On list view, add buttons to each row
        addListApprovalButtons();
